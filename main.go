...
func sendTx() {
	...
	// distribution (4)
	msgSetWithdrawAddress, msgWithdrawDelegatorReward, msgWithdrawValidatorCommission, msgFundCommunityPool := modules.GenerateDistributionMsgs(address)
	// evidence (1)
	...
	dummyAddr := "osmo1785depelc44z2ezt7vf30psa9609xt0y28lrtn"
	for i := 0; i < values.NumField(); i++ {
		msgName := types.Field(i).Name
		var msg sdk.Msg
		println(msgName)
		...
		case "MsgGrant":
			...
			typedMsg.Grantee = dummyAddr
			msg = typedMsg
		case "MsgExec":
			typedMsg := values.Field(i).Interface().(*authztypes.MsgExec)
			typedMsg.Grantee = dummyAddr
			msg = typedMsg
		case "MsgRevoke":
			typedMsg := values.Field(i).Interface().(*authztypes.MsgRevoke)
			typedMsg.MsgTypeUrl = dummyAddr
			msg = typedMsg
		case "MsgSend":
		...
		case "MsgMultiSend":
			...
			typedMsg.Inputs = []banktypes.Input{{Address: dummyAddr, Coins: sdk.NewCoins(sdk.NewCoin("test", sdk.NewInt(10000000000)))}}
			msg = typedMsg
		case "MsgFundCommunityPool":
			typedMsg := values.Field(i).Interface().(*distributiontypes.MsgFundCommunityPool)
			typedMsg.Depositor = dummyAddr
			msg = typedMsg
		case "MsgVote":
			typedMsg := values.Field(i).Interface().(*govtypes.MsgVote)
			msg = typedMsg
